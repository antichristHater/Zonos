...
class Zonos(nn.Module):
    ...
    def from_local(
        ...
        model = cls(config, backbone_cls).to(device, torch.float32)#, torch.bfloat16)
    ...
    def make_speaker_embedding(self, wav: torch.Tensor, sr: int) -> torch.Tensor:
        ...
        return spk_embedding.unsqueeze(0).to(torch.float32)
    ...
    def setup_cache(self, batch_size: int, max_seqlen: int, dtype: torch.dtype = torch.float32) -> InferenceParams:#.bfloat16) -> InferenceParams:
